...
/*   Updated: 2020/07/25 03:49:04 by lnoaille         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void draw_sprite(t_img *img, t_draw_sp *dsp)
{
	...
	while (i < dsp->nb_sprite)
	{
		...
		int sprite_height = ft_abs((int)(img->res_y / transform_y)) * 1.5;
		...
		int sprite_width = ft_abs((int)(img->res_y / transform_y)) * 1.5;
		...
		{
			int texX = (int)(256 * (stripe - (-sprite_width / 2 + sprite_screenx)) * sp->width / sprite_width) / 256;
			if (transform_y > 0 && stripe > 0 && stripe < img->res_x && transform_y < img->z_buffer[stripe])
			{
				int y = draw_start_y;
